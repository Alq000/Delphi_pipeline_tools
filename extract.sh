...
# Arguments (Shifted because PREFIX is now hardcoded)
NUM_EVENTS=$1
CONFIG_FILE=$2
...
podman exec -w /work "$CONTAINER_ID" /bin/bash -c "
    ...
    # The script hardcodes a check for these specific variables.
    ...
    # 'data' is now hardcoded as the second argument here
    ./run_pipeline.sh $NUM_EVENTS data /work/ $CONFIG_FILE && \
...
# Check if data.root exists using a bash sub-call
if podman exec "$CONTAINER_ID" /bin/bash -c "[[ -f /work/data.root ]]"; then
    # Using the hardcoded 'data' prefix for the .sdst filename
